...
//   echo $selfname;exit;
                      ...
                      ON tbl_user.id=apply_ride.userid WHERE apply_ride.driver_name ='$selfname' AND apply_ride.remarks != 'complete' AND apply_ride.status='approve' ";
...
<th  style="  border:1px solid black;">Current locaction</th>
           ...
           <?php while($row = $result->fetch_assoc()){ ?>
              ...
              <td style="  border:1px solid black;"><?php echo $row['time'];?></td>
               ...
               <?php echo "<td><a onClick=\"javascript: return confirm('Are you sure complete this ride?')\"href='?complete=".$row['id']."'>Complete</a></td>"; ?> 
      ...
      if (isset($_GET['complete'])) {
         ...
         $complete_query = "UPDATE apply_ride   SET remarks='complete'  WHERE id='$id'";
         $run_complete_query = mysqli_query($conn, $complete_query) or die (mysqli_error($conn));
         if (mysqli_affected_rows($conn) > 0) {
             echo "<script>alert('Ride Complete!');
             window.location.href='selfride.php';</script>";
         }
         else {
